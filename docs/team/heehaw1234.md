...
* **Enhancements implemented**:
  * Strengthened the robustness of the command-line interface by centralizing error validation inside `CommandHelper`. This caught critical edge cases such as integer overflow `NumberFormatException`s in task indices, unrecognized CLI flags, and silent fallbacks on missing SKU searches.
  * Set up logging infrastructure via `ItemTaskerLogger`, which suppresses red `stderr` log output in the CLI by redirecting logs to a file handler — improving user experience when running the `.jar`. Subsequently expanded system observability by integrating `java.util.logging` traces (INFO and FINE levels) deeply into `SKUTask`, `SKUTaskList`, and the `find` workflow inside `ViewCommandHandler`.
  * Hardened domain logic by introducing strict defensive programming strategies, wrapping `SKUTask` setters with null-guard assertions and implementing explicit bounds checking in `SKUTaskList` list operations to gracefully handle out-of-bounds indices.
  * Wrote comprehensive JUnit tests for `SKUTaskTest`, `SKUTaskListTest`, and `FindCommandTest` to verify core operations and edge cases in parsing and data storage. Expanded this suite by an additional 10 targeted positive and negative test cases to cover priority enumeration parsing, formatted `toString()` conversions, and complex combined-filter searches.
...
* **Contributions to the DG**:
  ...
  * Kept architectural diagrams up to date by tracking the transition of logic from `CommandRunner` to `TaskCommandHandler` & `ViewCommandHandler`. Standardized styling across all 15 PlantUML architecture diagrams by globally stripping default class attributes (`hide circle`) to improve aesthetic consistency.
  * Refreshed `listtasks-sequence.puml` and Exception documentation tables to precisely model the new explicit error branches (such as `InvalidFilterException` and `SKUNotFoundException`) injected during robustness refactoring.
  * Structured and categorized over two dozen user stories into the Appendix B tables, explicitly decoupling them into v1.0 and v2.0 feature tiers to demonstrate development velocity and requirements scoping.
